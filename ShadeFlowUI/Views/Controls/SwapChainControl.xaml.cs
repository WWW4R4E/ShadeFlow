...
namespace ShadeFlow.Views.Controls
{
    public sealed partial class SwapChainControl : SwapChainPanel
    {
        ...
        // 输入状态跟踪
        private bool _isMouseMiddleButtonPressed = false;
        ...
        private void OnPointerReleased(object sender, PointerRoutedEventArgs e)
        {
            ...
            if (point.Properties.PointerUpdateKind == Microsoft.UI.Input.PointerUpdateKind.MiddleButtonReleased)
            {
                _isMouseMiddleButtonPressed = false;
            }
        }
        ...
        {
            var point = e.GetCurrentPoint(this);
            var delta = (float)point.Properties.MouseWheelDelta;
            ...
            {
                var distance = ShadeFlowNative.GetCameraDistance();
                var newDistance = distance - delta * 0.001f;
                if (newDistance < 0.1f) newDistance = 0.1f;
                if (newDistance > 10.0f) newDistance = 10.0f;
                ShadeFlowNative.SetCameraDistance(newDistance);
                e.Handled = true;
            }
        }
